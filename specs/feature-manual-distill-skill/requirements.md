...
**And** the skill pipes a payload `{ transcript_path, cwd, session_id, reason: "manual" }` into `scripts/vault-distill.sh`
...
| FR7 | Read `vaultPath` from config; locate the note the hook just wrote by scanning `$VAULT/claude-memory/sessions/**/*.md` for the newest mtime | Must | Slug derivation is owned by the hook, not re-implemented in the skill |
| FR8 | Report: transcript used, output note path, whether it is a fallback stub | Must | AC1, AC7 |
...
| **Security** | Slug sanitization is delegated entirely to the hook; the skill does not handle file paths from prompt content |
...
| **Success output** | Transcript path + note path + fallback-stub flag |
...
| Skill stdout | Transcript path, note path, fallback-stub marker |
...
- [ ] `scripts/vault-distill.sh` — the workhorse (`feature-session-distillation-hook` / #11)
